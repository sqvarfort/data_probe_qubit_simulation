...
In this report, we we have presented results obtained from simulating a system of one probe qubit and four data qubits based on the setup proposed in \cite{OGorman2016}. We found that physically motivated errors of real-world magnitude did not severely impact the performance of this small scale device, and both the phase error and reduced stabiliser measurement probabilities were within the thresholds set out in \cite{OGorman2016} [REALLY?]. We found that the ratio between the coupling constant $J$ and the detuning $\Delta$ should be of the order $\Delta r^3/J \ge 10^4$ to avoid flip-flopping of the data qubits. Furthermore, we characterised the impact of changing physical parameters, such as the separation between the probe and data qubits $d$ and the in-plane qubit lattice constant $D$ on the parity measurement time. Out of the possible candidate qubit species, donors in silicon show the best prospects for a large scale implementation where we want to highlight the long coherent clock transitions of bismuth and possibilities of state transfer to the nuclear spin. Dephasing turned out to be the most crucial parameter for the performance of the system. To summarise, our simulations show that realistic errors that arise in an experimental implementation stay below the thresholds for fault-tolerance. 
